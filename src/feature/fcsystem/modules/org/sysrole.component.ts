...
import { FctreeComponent, ParentlistComponent } from 'fccomponent2';
    ...
    init(): void {
        ...
        SysroleBusiness.getRole(SysroleBusiness.appService.moduleId).subscribe(result => {
            if (result.CODE === '0') {
                ...
                //初始化列表默认显示第一个角色的用户信息
                // let con2 = {'and': "[{ 'ROLEID':'" + this.selectedRoleId + "' }, { 'PID': '" + this.selectedRolePid + "' }]"};
                // 根据角色id和产品的pid筛选出其他角色的用户（添加弹窗的内容）
                let con3: any = {
                    WHERE: '{ROLEID:{eq:"' + this.selectedRoleId + '"},PID:{eq:"' + this.selectedRolePid + '"}}'
                }
                this.condition = JSON.stringify(con3);
    ...
    listEvent(event: FCEVENT) {
        switch (event.eventName) {
            ...
            case 'select':
                ...
                let con3 = {WHERE:'{ROLEID: { eq: "' + event.param.ROLEID + '" }, PID: { eq: "' + this.selectedRolePid + '" } }'};
                this.condition = JSON.stringify(con3);
        ...
        if (this.selectedObject && this.selectedObject.ROLEID) {
            ...
            SysroleBusiness.getAuthByRoleid(roleId).subscribe(result => {
                if (result.CODE === '0') {
                    ...
                    if (isSelect&&this.menutree.fcTree) {
            ...
            case 'success':
                ...
                SysroleBusiness.saveRoleUser(seletedUser, this.seletedRoleObj.ROLEID).subscribe(result => {
                    if (result.CODE === '0') {
                        ...
                        let con ={WHERE:"{ROLEID: {eq:'" + this.seletedRoleObj.ROLEID + "},PID:{eq:'"+this.selectedRolePid+"'}} "};
                        //刷新角色用户
