    ...
    */
    ...
    initOrgData() {
        ...
        if (CacheService.getS("DefaultCompanyDim")) {
            ...
            CacheService.setS("higherCompanyDialogDim", this.compabyDimObj.CODE);


            //待数据请求成功后，根据默认维度和默认时间加载树和列表
            this.treeOptions = SyscompanyBusiness.cloneTreeObj(this.compabyDimObj["CODE"], this.senDateToString);
            //初始化列表配置
            // this.listOptions = CommonService.cloneObj(SyscompanyBusiness.fclistdataOption);
            // this.checkTree(null);
            //如果树结构不为空时,初始化树结构和列表
    ...
    checkTree(data: any) {
        ...
        if (data !== null) {
            ...
            // let con: any = {
            //     WHERE: "SPARENT_CODE =" + "'" + data.SCOMPANY_CODE + "'" + ' ' + "AND SDIM_CODE=" + "'" + this.compabyDimObj.CODE + "' " + "AND REND_DATE >='" + this.senDateToString + "'",
            //     ORDER: 'NDISPLAYNO'
            // } 
            let con: any = {
                WHERE: "{SPARENT_CODE:{eq:'" + data.SCOMPANY_CODE + "'},SDIM_CODE:{eq:'" + this.compabyDimObj.CODE + "'}}",
                ORDER: 'NDISPLAYNO'
            }
            this.condition = JSON.stringify(con);
