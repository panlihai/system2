...
@Component({
  ...
  :host ::ng-deep .content-wrap>div>.fc-content2{
    ...
    height: 100%;
    box-sizing: border-b ox;    position: relative;
  }
  ...
  :host ::ng-deep .fc-layout-tab .has-navsubtab .content-main{
    height:calc(100% - 73px);
  }

  :host ::ng-deep .fc-layout-tab-onelayertab .content-main{
  ...
  :host ::ng-deep .fc-theme-tab .fc-navbar-exit{
...
export class LayoutComponent implements OnInit {
  ...
  isLoading='N';
  constructor() {
    CommonService.subscribe("startLoading",()=>{
      this.isLoading = 'Y';
    });
    CommonService.subscribe("endLoading",()=>{
      this.isLoading = 'N';
    });
    //初始化消息配置
    ...
    SystemBusiness.getProduct(FCCONFIG.pid).subscribe(result => {
      if (result.CODE === '0') {
  ...
  ngOnInit() {
    ...
    CommonService.event('selectedMenu', {
      ...
      PID: FCCONFIG.pid, MENUTYPE: 'INURL', MENUNAME: '首页', MENUICON: 'fc-icon-home'
    });
    // 事件订阅，点击tab的时候
    ...
    // SystemBusiness.navigate(["/" + FCCONFIG.pid.toLocaleLowerCase() + "/home"]);
  }
  /**
   * 获取当前消息
   */
  ...
  navbarEvent(event: FCEVENT) {
    switch (event.eventName) {
      ...
      case 'logout'://登出
        ...
        break;
      case 'editUser'://修改密码
        SystemBusiness.navigate(["/profile"]);
        break;
  ...
  selectedtabmain(menu: any, refresh?: string) {
    ...
    if (menu.P_CHILDMENUS && menu.P_CHILDMENUS.length !== 0) {
      ...
      SystemBusiness.navigate(["/" + FCCONFIG.pid.toLocaleLowerCase() + "/" + menu.ROUTER], {
  ...
  selectedtabsub(menu: any) {
    SystemBusiness.msgService.startLoading();
    SystemBusiness.navigate(["/" + FCCONFIG.pid.toLocaleLowerCase() + "/" + menu.ROUTER], { queryParams: { refresh: 'Y', MENUICON: menu.MENUICON } }).then(result=>{
      SystemBusiness.msgService.endLoading();
    });
  }
  /**
   *  菜单事件
